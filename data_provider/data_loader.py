...
class Dataset_original(Dataset):
    def __init__(self, args, flag='train', label_scaler=None, tokenizer=None, eval_cycle_max=None, eval_cycle_min=None, total_prompts=None, 
        ...
        if cached is not None:
            self.total_charge_discharge_curves, self.total_curve_attn_masks, self.total_labels, self.unique_labels, self.class_labels, self.total_dataset_ids, self.total_cj_aug_charge_discharge_curves, self.total_seen_unseen_IDs = cached
        else:
            _result = self.read_data()
            self.total_charge_discharge_curves, self.total_curve_attn_masks, self.total_labels, self.unique_labels, self.class_labels, self.total_dataset_ids, self.total_cj_aug_charge_discharge_curves, self.total_seen_unseen_IDs = _result
            self._save_cache(_result)
    ...
    def _get_cache_path(self):
        ds_name = {
            'NAion': 'NA-ion',
            'NAion42': 'NA-ion42',
            'NAion2024': 'NA-ion2024',
        }.get(self.dataset, self.dataset)
        
        cache_dir = "/work/huy.leminh/code/Dr.Huy/AI Battery/data/dataset_cache"
        os.makedirs(cache_dir, exist_ok=True)
        align_suffix = "_aligned" if getattr(self.args, 'align_features', False) else "_unaligned"
        fname = f"{ds_name}_{self.flag}_seq{self.seq_len}_cd{self.charge_discharge_len}_ec{self.early_cycle_threshold}{align_suffix}.npz"
        return os.path.join(cache_dir, fname)

    def _save_cache(self, data_tuple):
        cache_path = self._get_cache_path()
        (total_charge_discharge_curves, total_curve_attn_masks, total_labels,
         unique_labels, class_labels, total_dataset_ids,
         total_cj_aug_charge_discharge_curves, total_seen_unseen_IDs) = data_tuple
        print(f'[Cache] Saving to {cache_path} ...')
        np.savez(
            cache_path,
            total_charge_discharge_curves=np.array(total_charge_discharge_curves),
            total_curve_attn_masks=np.array(total_curve_attn_masks),
            total_labels=np.array(total_labels),
            unique_labels=np.array(unique_labels),
            class_labels=np.array(class_labels),
            total_dataset_ids=np.array(total_dataset_ids),
            total_cj_aug_charge_discharge_curves=np.array(total_cj_aug_charge_discharge_curves),
    ...
    def _load_cache(self):
        ...
        result = (
            list(d['total_charge_discharge_curves']),
            list(d['total_curve_attn_masks']),
            d['total_labels'],
            list(d['unique_labels']),
            list(d['class_labels'].astype(int)),
            list(d['total_dataset_ids'].astype(int)),
            list(d['total_cj_aug_charge_discharge_curves']),
            list(d['total_seen_unseen_IDs'].astype(int)),
        )
        print(f'[Cache] Loaded {len(result[2])} samples.')
        return result
    ...
    def get_charge_discharge_curves(self, file_name, df, early_cycle_threshold, nominal_capacity):
        ...
        # Get initial capacity from cycle 1 if available for Method 1 scaling
        first_cycle_df = df.loc[df['cycle_number'] == 1] if 1 in df['cycle_number'].unique() else df
        initial_capacity = np.nanmax(first_cycle_df['charge_capacity_in_Ah'].values) if len(first_cycle_df) > 0 else nominal_capacity
        if np.isnan(initial_capacity) or initial_capacity <= 0:
            initial_capacity = nominal_capacity
        ...
        for cycle in range(1, early_cycle_threshold+1):
            if cycle in df['cycle_number'].unique():
                ...
                if getattr(self.args, 'align_features', False):
                    # Method 1 aligned voltage normalization: min-max cycle scaling
                    v_max = np.nanmax(voltage_records)
                    v_min = np.nanmin(voltage_records)
                    if not np.isnan(v_max) and not np.isnan(v_min) and v_max > v_min:
                        voltage_records = (voltage_records - v_min) / (v_max - v_min)
                    else:
                        denom = v_max if (not np.isnan(v_max) and v_max != 0) else 1.0
                        voltage_records = voltage_records / denom
                    voltage_records = voltage_records.reshape(1, self.charge_discharge_len)

                    current_records = current_records.reshape(1, self.charge_discharge_len) / nominal_capacity
                    
                    # Method 1 aligned capacity normalization: scale relative to initial capacity
                    capacity_in_battery = capacity_in_battery.reshape(1, self.charge_discharge_len) / initial_capacity
                else:
                    # Original scaling (unaligned)
                    v_max = np.nanmax(voltage_records)
                    denom = v_max if (not np.isnan(v_max) and v_max != 0) else 1.0
                    voltage_records = voltage_records.reshape(1, self.charge_discharge_len) / denom
                    current_records = current_records.reshape(1, self.charge_discharge_len) / nominal_capacity
                    capacity_in_battery = capacity_in_battery.reshape(1, self.charge_discharge_len) / nominal_capacity
